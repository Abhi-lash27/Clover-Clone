...
module.exports = () => {
  ...
  const mongooseConnect = () => {
    ...
    const uri = process.env.MONGO_URI; // Use Atlas URI from .env
    if (!uri) {
      console.error('MONGO_URI not set in .env'.red);
      return;
    }
        ...
        User.findOne({ email }).then((user) => {
          if (!user) {
            argon2.hash(password).then((hash) => {
              new User({ username, email, password: hash, firstName, lastName, level: 'root' }).save();
            });
          } else {
            argon2.hash(password).then((hash) => {
